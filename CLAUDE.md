...
사주풀이 웹앱('명리학 도사' 미연시풍, Cloudflare Pages 배포) 설계 → `dosa-app/README.md`. 지식 주입(트레이닝)은 **/feed 스킬**(`.claude/skills/feed/`) — 유튜브 링크 검수→주입→색인 갱신→보완 보고. **절대 원칙**: 근거는 코퍼스 출처 필수, 자료 선택은 검색(RAG)이 아니라 만세력 계산 결과가 키가 되는 결정론 조회, LLM은 서술층일 뿐(없어도 조립식 리포트로 완결 동작), 지식은 요소별 정제 유닛으로 증류(문헌 이견은 관점차이로 병기).
